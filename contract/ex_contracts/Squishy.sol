...
contract SquishySquad is ERC721Enumerable, Ownable {
    ...
    address public constant founderAddress =
        0x52d03A5e20551D7B0393843594D5F5465ccD8d88;

    address private constant launchpadAddress =
        0x6BCBE6c086cC66806B0b7B4A53409058d85F61c8;
    address private constant donationAddress =
        0xDfd143aE8592e8E3C13aa3E401f72E1ca7deAED0;
    ...
    {
        require(
            reservedClaimed + amount <= RESERVED_SQUISHES,
            "Minting would exceed max reserved squishes"
        );
        require(recipient != address(0), "Cannot add null address");
        require(
            totalSupply() + amount <= MAX_SQUISHES,
    ...
    function addToHandpickedPresale(address[] calldata addresses)
        external
        onlyOwner
    {
    ...
    function addToPartnershipPresale(address[] calldata addresses)
        external
        onlyOwner
    {
        for (uint256 i = 0; i < addresses.length; i++) {
    ...
    function checkHandpickedEligibility(address addr)
        external
        view
        returns (bool)
    {
    ...
    function checkPresaleEligibility(address addr)
        external
        view
        returns (bool)
    {
    ...
    function getHandpickedMintsClaimed(address addr)
        external
        view
        returns (uint256)
    {
    ...
    function getPresaleMintsClaimed(address addr)
        external
        view
        returns (uint256)
    {
    ...
    function mintPartnershipPresale() external payable whenPresaleStarted {
        require(
            ...
            "You are not eligible for the partnership presale"
        );
        require(
            totalSupply() + 1 <=
                MAX_SQUISHES - (RESERVED_SQUISHES - reservedClaimed),
            "Minting would exceed max supply"
        );
        require(
            _totalClaimedPresale[msg.sender] + 1 <= PRESALE_MAX_MINT,
            "Only 1 mint allowed"
        );
    ...
    function mintHandpickedPresale(uint256 amountOfSquishes)
        external
        payable
        whenPresaleStarted
    {
        require(
            _handpickedEligible[msg.sender],
            "You are not eligible for the handpicked presale"
        );
        require(
            totalSupply() + amountOfSquishes <=
                MAX_SQUISHES - (RESERVED_SQUISHES - reservedClaimed),
            "Minting would exceed max supply"
        );
        require(
            _totalClaimedHandpicked[msg.sender] + amountOfSquishes <=
                HANDPICKED_MAX_MINT,
            "Purchase exceeds max allowed"
        );
        require(amountOfSquishes > 0, "Must mint at least one squish");
        require(
            PRICE * amountOfSquishes <= msg.value,
    ...
    function mint(uint256 amountOfSquishes)
        external
        payable
        whenPublicSaleStarted
    {
        ...
        require(
            totalClaimed[msg.sender] + amountOfSquishes <= MAX_SQUISHES_MINT,
            "Purchase exceeds max allowed per address"
        );
        require(amountOfSquishes > 0, "Must mint at least one squish");
        require(
            amountOfSquishes <= MAX_PER_TX,
    ...
    function bulkPurchase(uint256 amountOfSquishes) external payable {
        ...
        require(amountOfSquishes > 0, "Must mint at least one squish");
        require(
            msg.sender == launchpadAddress || msg.sender == donationAddress,
            "Must be launchpad or donation wallet address"
        );
        require(
